...
# UI Node References
@onready var authority_label: Label = $UI/JarlInfo/VBoxContainer/AuthorityLabel # [cite: 438, 479]
@onready var renown_label: Label = $UI/JarlInfo/VBoxContainer/RenownLabel # [cite: 439]
@onready var end_year_button: Button = $UI/Actions/VBoxContainer/EndYearButton # [cite: 441, 481]
@onready var region_info_panel: PanelContainer = $UI/RegionInfo # [cite: 442]
@onready var region_name_label: Label = $UI/RegionInfo/VBoxContainer/RegionNameLabel # [cite: 443]
@onready var launch_raid_button: Button = $UI/RegionInfo/VBoxContainer/LaunchRaidButton # [cite: 444, 480]
@onready var settlement_button: Button = $UI/Actions/VBoxContainer/SettlementButton # [cite: 445, 483]
@onready var tooltip: PanelContainer = $UI/Tooltip # [cite: 446, 482]
@onready var tooltip_label: Label = $UI/Tooltip/Label

@onready var regions_container: Node2D = $Regions # [cite: 434]

var selected_region_data: WorldRegionData # [cite: 478]
var selected_region_node: Region = null

func _ready() -> void:
	# Connect to the DynastyManager
	DynastyManager.jarl_stats_updated.connect(_update_jarl_ui) # [cite: 486]
	
	# Connect to all child regions [cite: 487]
	...
	# Connect local UI buttons [cite: 488]
	...
	# Initialize UI
	_update_jarl_ui(DynastyManager.get_current_jarl()) # [cite: 489]
...
func _update_jarl_ui(jarl: JarlData) -> void:
	...
	authority_label.text = "Authority: %d / %d" % [jarl.current_authority, jarl.max_authority] # [cite: 507]
	renown_label.text = "Renown: %d" % jarl.renown # [cite: 508]
...
func _on_region_selected(data: WorldRegionData) -> void:
	...
	selected_region_data = data # [cite: 493]
	
	# Update the UI panel [cite: 494]
	region_name_label.text = data.display_name
	launch_raid_button.text = "Raid: %s (Cost: %d)" % [data.display_name, data.base_authority_cost] # [cite: 495]
	
	# Check if Jarl can afford this action [cite: 496]
...
func _on_end_year_pressed() -> void:
	DynastyManager.end_year() # [cite: 502]
...
func _on_settlement_pressed() -> void:
	EventBus.scene_change_requested.emit(settlement_bridge_scene_path) # [cite: 504]
